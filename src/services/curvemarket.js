...
// Pre-graduation BULL price, computed from the Pons bonding curve.
//
// Until BULL graduates off the Pons V2 bonding curve there is no Uniswap pool,
...
// Multiplying the two gives a real USD price for BULL today:
//
//   GET {ponsApi}/api/pons-v2-market/{token}/chart?range=1d
//     -> { points: [{ t, price, ... }] }        price = quote per BULL
...
// pair for BULL itself takes over (see routes/stats.js merge order) and this
...
/** Pure: latest curve price (quote asset per BULL) from a Pons chart payload, or null. */
 ...
 * empty — the token simply has no price yet. But a missing quote/USD price is
 * always an upstream glitch (these tokenized stocks are all deeply listed), so
 * it THROWS:
...
function combinePrices(curvePrice, quoteUsd) {
  ...
  if (quoteUsd === null) throw new Error('quote price unavailable from DexScreener');
...
async function fetchCurveMarket() {
  ...
  const [chart, quote] = await Promise.all([
    ...
    getQuotePrice(), // shared cached quote/USD read (see quoteprice.js)
  ]);
