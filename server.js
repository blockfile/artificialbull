...
app.use((err, req, res, next) => {
  if (err && err.corsRejected) {
    ...
    if (!loggedBlockedOrigins.has(origin)) {
      ...
      console.warn(`[ryzenkitty] blocking CORS origin: ${origin}`);
    }
    return res.status(403).json({ error: 'origin not allowed' });
  }
  console.error('[ryzenkitty] request error:', err);
...
if (require.main === module) {
  server = app.listen(config.port, () => {
    console.log(`[ryzenkitty] listening on http://localhost:${config.port}`);
    console.log(
      `[ryzenkitty] token=${config.tokenSymbol} address=${config.tokenAddress || '(not set — stats will be null)'}`
    );
    console.log(`[ryzenkitty] cors=${config.corsOrigins.join(', ')}`);
  });

  const shutdown = (signal) => {
    console.log(`\n[ryzenkitty] ${signal} received, shutting down`);
